...
def verify_image(args):
    ...
    try:
        ...
        nc = 1
        msg = f"{prefix}WARNING ⚠️ {im_file}: ignoring corrupt image/label: {e} (verify_image)"
    return (im_file, cls), nf, nc, msg


def verify_image_label(args):
    """Verify one image-label pair."""
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, renumber = args
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                lb = [[renumber.get(int(x[0]), int(x[0]))] + x[1:] for x in lb]
                lb = [x for x in lb if x[0] >= 0]
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    classes = np.array([x for x in lb], dtype=np.float32)
                    segments = [
